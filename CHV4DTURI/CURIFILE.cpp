...
namespace CTURI
{
	...
	CURIFILE::CURIFILE(std::string const& uri)
	{
		tagURI = uri;
	...
	CURIFILE::CURIFILE(CURIFILE const& uri)
	{
		tagURI = uri.tagURI;
	...
	void CURIFILE::operator = (std::string const& uri)
	{
		tagURI = uri;
	...
	void CURIFILE::operator = (CURIFILE const& uri)
	{
		tagURI = uri.tagURI;
	...
	bool CURIFILE::operator == (std::string const& uri) const
	{
		CURIFILE var{};
	...
	bool CURIFILE::operator == (CURIFILE const& uri) const
	{
		return tagURI.compare(uri.tagURI) == 0;

	}

	std::string CURIFILE::operator()() const
	{
		return tagURI;

	}

	CTURI::SURIFILE CURIFILE::GetResource() const
	{
		return resource;
